...
public class ActualEePoseSubscriber : MonoBehaviour
{
    public string topic = "/robot/ee_pose";
    ...
    void OnPose(PoseStampedMsg msg)
    {
        ...
        actualBox.position = RosToUnityPosition(posRos);

        if (applyRotation)
        {
            Quaternion rotRos = new Quaternion((float)q.x, (float)q.y, (float)q.z, (float)q.w);
            actualBox.rotation = RosToUnityRotation(rotRos) * rotationOffset;
        }
    }
